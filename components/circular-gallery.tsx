...
function createTextTexture(gl: any, text: string, font = "bold 30px Figtree", color = "#eaeaea") {
...
class Title {
  ...
  constructor({ gl, plane, renderer, text, textColor = "#eaeaea", font = "bold 28px Figtree" }: any) {
    autoBind(this)
    this.gl = gl
    this.plane = plane
...
class App {
  ...
  createMedias(items: Array<{ image: string; text: string }>, bend = 2.5, textColor: string, borderRadius: number, font: string) {
  ...
  update() {
    ...
    const direction = this.scroll.current > this.scroll.last ? ("right" as const) : ("left" as const)
